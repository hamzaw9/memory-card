...
const ImageCard = ({
  ...
  handleCurrentImage: (id: number) => void;
  gameOver: boolean;
}) => {
  const [images, setImages] = useState<Character[]>([]);
  const [originalImages, setOriginalImages] = useState<Character[]>([]);
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        ...
        const sliced = data.results.slice(0, 12);

        setOriginalImages(sliced);
        setImages(sliced);
        setLoading(false);
      } catch (error) {
  ...
  return (
    <div className="flex flex-wrap justify-center sm:justify-between gap-5 p-5 px-4">
      {images.map((character) => (
        <div
          onClick={() => handleImageShuffle(character.id)}
          key={character.id}
        ...
        >
          ...
          <div className="mt-3 w-full px-2">
            <p className="text-center font-medium text-[#fffffe] truncate font-lobster text-xl">
